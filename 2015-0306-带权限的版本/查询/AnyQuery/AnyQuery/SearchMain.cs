...
namespace AnyQuery
{
    public partial class SearchMain : Form
    {
        ...
        public SearchMain(string roleId)
        {
            InitializeComponent();
            this.roleId = roleId;
        }
        ...
        public void ConnectDBAndInitData()
        {
            try
            {
                ...
                dr = dbHelp.ConnectDBAndInitData(dbHelp.MInitData(roleId));//!!!!!!!!!!!!!!！！！！！！！！！！！！!!!!!!!!!!!!!!!!!!!!!!!!!!可能会有问题
                if (dr == null)
                {
                ...
                else
                {
                    while (dr.Read())
                    {
                        selecttable.Items.Add(dr["table_name"].ToString());
                    }
        ...
        //关闭中，将会请求用户确认是否退出。
        private void SearchMain_FormClosing(object sender, FormClosingEventArgs e)
        {
